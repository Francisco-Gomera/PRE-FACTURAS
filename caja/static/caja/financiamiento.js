(function () {
  ...
  function buildSavePayload(detailRows) {
    ...
    return {
      record_lookup: String(
        currentNoDoc ||
        (fieldNo && fieldNo.value ? fieldNo.value : "") ||
        (fieldNoFactura && fieldNoFactura.value ? fieldNoFactura.value : "")
      ).trim(),
  ...
  function buildFinanciamientoPrintHtml(copyCount) {
    ...
    return (
      ...
      ".signatures{margin-top:auto;display:grid;grid-template-columns:1fr 1fr;gap:80px;font-size:12px;padding-top:18px;align-items:end;}" +
      ".sign-box{display:flex;flex-direction:column;justify-content:flex-end;align-items:center;min-height:52mm;}" +
      ".sign-space{height:28mm;display:flex;align-items:flex-end;justify-content:center;width:100%;overflow:hidden;}" +
  ...
  function openFinanciamientoPrint(copyCount) {
  ...
  function saveRecord() {
    ...
    fetch(config.guardarUrl, {
      ...
      .then(function (result) {
        ...
        applyRecord(result.data.record || {});
        openPrintModal();
      })
      .catch(function () {
  ...
  function applyRecord(data) {
    ...
    currentFacturaBase = data && data.editable ? { no_doc: prestamo.no_factura || prestamo.no_doc || "" } : null;
    agreementDatesByCuota = {};
    syncShortcutButtonsEnabled();
    ...
    setFieldValue(fieldMonto, prestamo.monto || 0, fmtMoney);
  ...
  if (btnPrintConfirm) {
    btnPrintConfirm.addEventListener("click", function () {
      var copies = normalizePrintCopies(fieldPrintCopies ? fieldPrintCopies.value : 2);
      closePrintModal();
      openFinanciamientoPrint(copies);
    });
  }
  if (fieldPrintCopies) {
